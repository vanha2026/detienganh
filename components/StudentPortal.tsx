...
const StudentPortal: React.FC<StudentPortalProps> = ({ onJoinRoom }) => {
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      // Tạo student ID từ tên + random
      const sId = studentId.trim() || `S${Date.now()}`;
      
      const student: StudentInfo = {
        id: sId,
        name: studentName.trim(),
        className: studentClass.trim() || undefined,
    ...
    <div className="min-h-screen flex items-center justify-center p-4" 
      ...
      <div className="w-full max-w-md">
            ...
            <>
              <div className="mb-6">
                ...
                <input
                  ...
                  onKeyPress={(e) => e.key === 'Enter' && handleCheckRoom()}
                />
              </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-teal-600">
                      Mã: <span className="font-mono font-bold">{room?.code}</span> • 
                      Thời gian: {room?.timeLimit} phút
                    </p>
                  </div>
                </div>
              </div>
        ...
        <p className="text-center text-teal-600 mt-6 text-sm">
          Liên hệ giáo viên nếu bạn chưa có mã phòng thi
        </p>
      </div>
    </div>
  );
